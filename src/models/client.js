...
function deriveEncryptionKey(secret, length) {
  const digest =
    length <= 32 ? "sha256" : length <= 48 ? "sha384" : length <= 64 ? "sha512" : false; // eslint-disable-line no-nested-ternary
  if (!digest) {
    ...
    if (client.clientSecret) {
      if (client.clientAuthMethod === "client_secret_jwt") {
        if (client.clientAuthSigningAlg) {
          ...
          (instance(provider).configuration("clientAuthSigningAlgValues") || []).forEach(
        ...
        .then((response) => {
          ...
          if (statusCode !== 204 && statusCode !== 200) {
            const error = new Error(
              `expected 204 No Content from ${this.backchannelClientNotificationEndpoint}, got: ${statusCode} ${STATUS_CODES[statusCode]}`
            );
      ...
      return request
        ...
        .then((response) => {
          ...
          if (statusCode !== 200 && statusCode !== 204) {
            const error = new Error(
              `expected 200 OK from ${this.backchannelLogoutUri}, got: ${statusCode} ${STATUS_CODES[statusCode]}`
            );
            error.response = response;
            throw error;
          }
