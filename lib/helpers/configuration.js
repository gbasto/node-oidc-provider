...
var _remove = _interopRequireDefault(require("./_/remove.js"));
var _merge = _interopRequireDefault(require("./_/merge.js"));
var _pick = _interopRequireDefault(require("./_/pick.js"));
var _set = _interopRequireDefault(require("./_/set.js"));
...
function featuresTypeErrorCheck({
  features
}) {
...
class Configuration {
  constructor(config) {
    Object.assign(this, (0, _merge.default)({}, this.defaults, (0, _pick.default)(config, ...Object.keys(this.defaults))));
  ...
  get defaults() {
    if (!(0, _weak_cache.default)(this).defaults) {
      (0, _weak_cache.default)(this).defaults = (0, _defaults.default)();
    }
    return (0, _weak_cache.default)(this).defaults;
  }
  set defaults(val) {
    if (val === undefined) {
      delete (0, _weak_cache.default)(this).defaults;
    }
  }
  ensureSets() {
    ['scopes', 'subjectTypes', 'extraParams', 'acrValues', 'clientAuthMethods', 'features.ciba.deliveryModes'].forEach(prop => {
      if (!((0, _get.default)(this, prop) instanceof Set)) {
        if (!Array.isArray((0, _get.default)(this, prop))) {
          throw new TypeError(`${prop} must be an Array or Set`);
        }
        const setValue = new Set((0, _get.default)(this, prop));
        (0, _set.default)(this, prop, setValue);
      }
    });
  }
  checkResponseTypes() {
    ...
    this.responseTypes.forEach(type => {
      ...
      if (parsed.has('none') && parsed.size !== 1 || parsed.has('token') && parsed.size === 1 || ![...parsed].every(Set.prototype.has.bind(supportedResponseTypes))) {
  ...
  collectScopes() {
    ...
    Object.entries(this.claims).forEach(([key, value]) => {
      if ((0, _is_plain_object.default)(value) || Array.isArray(value)) {
  ...
  collectPrompts() {
    ...
    this.interactions.policy.forEach(({
      name,
      requestable
    }) => {
      if (requestable) {
  ...
  checkAllowedJWA() {
    Object.entries(this.enabledJWA).forEach(([key, value]) => {
      ...
      value.forEach(alg => {
        if (!_index.JWA[key].includes(alg)) {
  ...
  setAlgs(prop, values, ...features) {
    if (features.length === 0 || features.every(feature => {
      if (Array.isArray(feature)) {
        return feature.some(anyFeature => (0, _get.default)(this.features, anyFeature));
      }
      return (0, _get.default)(this.features, feature);
    })) {
      this[prop] = values;
    } else {
      this[prop] = [];
  ...
  defaultSigAlg() {
    ...
    if (!this.clientAuthMethods.has('client_secret_jwt')) {
      (0, _remove.default)(this.clientAuthSigningAlgValues, filterHS);
    ...
    if (!this.clientAuthMethods.has('private_key_jwt')) {
      (0, _remove.default)(this.clientAuthSigningAlgValues, filterAsymmetricSig);
  ...
  checkDependantFeatures() {
    ...
    if (features.registration.enabled && features.registration.policies && !features.registration.initialAccessToken) {
  ...
  checkRequestMergingStrategy() {
    if (!requestObjectStrategies.has(this.features.requestObjects.mode)) {
      throw new TypeError(`'mode' must be ${formatters.formatList([...requestObjectStrategies], {
        type: 'disjunction'
      })}`);
    }
  }
    ...
    if (!this.features.fapi.enabled) {
      ...
      this.features.fapi.profile = (...args) => {
        ...
        if (profile && !fapiProfiles.has(profile)) {
          throw new TypeError(`'profile' must be ${formatters.formatList([...fapiProfiles], {
            type: 'disjunction'
          })}`);
        }
  ...
  checkAuthMethods() {
    const authMethods = new Set(['none', 'client_secret_basic', 'client_secret_jwt', 'client_secret_post', 'private_key_jwt']);
  ...
  logDraftNotice() {
    ...
    Object.entries(this.features).forEach(([flag, {
      enabled,
      ack
    }]) => {
      const {
      ...
      const draft = _features.DRAFTS.get(flag);
      if (draft && enabled && !_features.STABLE.has(flag) && (Array.isArray(draft.version) ? !draft.version.includes(ack) : ack !== draft.version)) {
...
var _default = exports.default = Configuration;