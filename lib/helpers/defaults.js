...
var _index2 = require("./interaction_policy/index.js");
...
function httpOptions(url) {
  return {
    signal: undefined,
    // defaults to AbortSignal.timeout(2500)
    agent: undefined,
    // defaults to node's global agents (https.globalAgent or http.globalAgent)
    dnsLookup: undefined,
    // defaults to `dns.lookup()` (https://nodejs.org/api/dns.html#dnslookuphostname-options-callback)
...
async function pairwiseIdentifier(ctx, accountId, client) {
  ...
  return crypto.createHash('sha256').update(client.sectorIdentifier).update(accountId).update(os.hostname()) // put your own unique salt here, or implement other mechanism
  .digest('hex');
}
...
function RefreshTokenTTL(ctx, token, client) {
  ...
  if (ctx && ctx.oidc.entities.RotatedRefreshToken && client.applicationType === 'web' && client.clientAuthMethod === 'none' && !token.isSenderConstrained()) {
  ...
  ctx.body = `<!DOCTYPE html>
    ...
    <body>
      <div class="container">
        ...
        ${Object.entries(out).map(([key, value]) => `<pre><strong>${key}</strong>: ${(0, _html_safe.default)(value)}</pre>`).join('')}
      </div>
    </body>
    </html>`;
}
...
function makeDefaults() {
  const defaults = {
    ...
    cookies: {
      ...
      names: {
        session: '_session',
        // used for main session reference
        interaction: '_interaction',
        // used by the interactions for interaction session reference
      ...
      long: {
        httpOnly: true,
        // cookies are not readable by client-side javascript
        overwrite: true,
        sameSite: 'none'
      },
      /*
     ...
     *
     */
    jwks: JSON.parse(JSON.stringify(_index.DEV_KEYSTORE)),
    /*
     * responseTypes
     *
     ...
     * ```
     */
    clientAuthMethods: ['client_secret_basic', 'client_secret_jwt', 'client_secret_post', 'private_key_jwt', 'none'],
    /*
     * ttl
     *
    ...
    enabledJWA: {
       ...
       * ```
       */
      idTokenEncryptionAlgValues: ['A128KW', 'A256KW', 'ECDH-ES', 'RSA-OAEP', 'RSA-OAEP-256', 'dir'],
      /*
       ...
       * ```
       */
      requestObjectEncryptionAlgValues: ['A128KW', 'A256KW', 'ECDH-ES', 'RSA-OAEP', 'RSA-OAEP-256', 'dir'],
      /*
       ...
       * ```
       */
      userinfoEncryptionAlgValues: ['A128KW', 'A256KW', 'ECDH-ES', 'RSA-OAEP', 'RSA-OAEP-256', 'dir'],
      /*
       ...
       * ```
       */
      introspectionEncryptionAlgValues: ['A128KW', 'A256KW', 'ECDH-ES', 'RSA-OAEP', 'RSA-OAEP-256', 'dir'],
      /*
       ...
       * ```
       */
      authorizationEncryptionAlgValues: ['A128KW', 'A256KW', 'ECDH-ES', 'RSA-OAEP', 'RSA-OAEP-256', 'dir'],
      /*
       ...
       * ```
       */
      dPoPSigningAlgValues: ['ES256', 'EdDSA']
    }
  };
  return defaults;
}
var _default = exports.default = makeDefaults;
const defaults = exports.defaults = makeDefaults();