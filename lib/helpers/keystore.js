...
class KeyStore {
  ...
  [filter](selector, scoring) {
    return this.#keys.filter(selector).sort((first, second) => keyscore(second, scoring) - keyscore(first, scoring));
  }
  add(key) {
    this.#keys.push(key);
  }
  ...
  async getKeyObject(jwk, alg) {
    ...
    const keyObject = await (0, _jose.importJWK)({
      ...jwk,
      alg
    });
...
var _default = exports.default = KeyStore;