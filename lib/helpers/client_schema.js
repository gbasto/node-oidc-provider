...
var validUrl = _interopRequireWildcard(require("./valid_url.js"));
var _errors = require("./errors.js");
var _sector_identifier = _interopRequireDefault(require("./sector_identifier.js"));
var _weak_cache = _interopRequireDefault(require("./weak_cache.js"));
var formatters = _interopRequireWildcard(require("./formatters.js"));
...
const {
  ...
  WEB_URI,
  WHEN
} = _index.CLIENT_ATTRIBUTES;
function isUndefined(value) {
  return value === undefined;
}
function getSchema(provider) {
  ...
  (0, _weak_cache.default)(provider).RECOGNIZED_METADATA = RECOGNIZED_METADATA;
  const ENUM = {
    ...ENUMS,
  ...
  class Schema {
    constructor(metadata, ctx, processCustomMetadata = !!configuration.extraClientMetadata.properties.length) {
      Object.assign(this, (0, _omit_by.default)((0, _pick.default)(DEFAULT, ...RECOGNIZED_METADATA), isUndefined), (0, _omit_by.default)((0, _pick.default)(metadata, ...RECOGNIZED_METADATA, ...configuration.extraClientMetadata.properties), isUndefined));
      this.required();
      this.booleans();
      this.whens();
      this.arrays();
      ...
      const responseTypes = [...new Set(this.response_types.map(rt => rt.split(' ')))].reduce((acc, val) => [...acc, ...val], []);
      ...
      }
      {
        const {
          length
        } = [this.tls_client_certificate_bound_access_tokens, this.dpop_bound_access_tokens].filter(Boolean);
        if (length > 1) {
      ...
      }
      {
        const {
          length
        } = [this.tls_client_auth_san_dns, this.tls_client_auth_san_email, this.tls_client_auth_san_ip, this.tls_client_auth_san_uri, this.tls_client_auth_subject_dn].filter(Boolean);
      ...
      (0, _sector_identifier.default)(this);
    ...
    invalidate(message, code) {
      ...
      throw new _errors.InvalidClientMetadata(message);
    }
    required() {
      ...
      if (this.subject_type === 'pairwise') {
        if (Array.isArray(this.grant_types) && this.grant_types.includes('urn:ietf:params:oauth:grant-type:device_code')) {
          checked.push('jwks_uri');
          if (Array.isArray(this.response_types) && this.response_types.length) {
            checked.push('sector_identifier_uri');
          }
        }
        if (Array.isArray(this.response_types) && this.response_types.length && Array.isArray(this.redirect_uris) && new Set(this.redirect_uris.map(uri => new URL(uri).host)).size > 1) {
          checked.push('sector_identifier_uri');
        }
      }
      ...
      const requireJwks = ['private_key_jwt', 'self_signed_tls_client_auth'].includes(this.token_endpoint_auth_method) || needsJwks.jws.test(this.request_object_signing_alg) || needsJwks.jws.test(this.backchannel_authentication_request_signing_alg) || needsJwks.jwe.test(this.id_token_encrypted_response_alg) || needsJwks.jwe.test(this.userinfo_encrypted_response_alg) || needsJwks.jwe.test(this.introspection_encrypted_response_alg) || needsJwks.jwe.test(this.authorization_encrypted_response_alg);
        ...
        if (this[prop] !== undefined) {
          ...
          (isAry ? this[prop] : [this[prop]]).forEach(val => {
            if (typeof val !== 'string' || !val.length) {
              this.invalidate(isAry ? `${prop} must only contain strings` : `${prop} must be a non-empty string if provided`);
            }
          });
        }
      });
        ...
        if (this[prop] !== undefined) {
          ...
          (isAry ? this[prop] : [this[prop]]).forEach(val => {
            ...
            if (!validUrl[method](val)) {
              this.invalidate(isAry ? `${prop} must only contain ${type} uris` : `${prop} must be a ${type} uri`);
            }
          });
        }
      });
      ...
      Object.entries(ENUM).forEach(([prop, fn]) => {
        ...
        if (this[prop] !== undefined) {
          ...
          if (isAry && !this[prop].every(val => only[method](val))) {
            if (length) {
              this.invalidate(`${prop} can only contain ${formatters.formatList([...only], {
                type: 'disjunction'
              })}`);
            } else {
            ...
            if (length) {
              this.invalidate(`${prop} must be ${formatters.formatList([...only], {
                type: 'disjunction'
              })}`);
            } else {
    ...
    jarPolicy() {
      ...
      const enabled = requestObjects.request || requestObjects.requestUri;
      if (enabled) {
