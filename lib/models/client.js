...
var _keystore = _interopRequireDefault(require("../helpers/keystore.js"));
var _snake_case = _interopRequireDefault(require("../helpers/_/snake_case.js"));
var _map_keys = _interopRequireDefault(require("../helpers/_/map_keys.js"));
...
var _client_schema = _interopRequireDefault(require("../helpers/client_schema.js"));
var _certificate_thumbprint = _interopRequireDefault(require("../helpers/certificate_thumbprint.js"));
var _sector_identifier = _interopRequireDefault(require("../helpers/sector_identifier.js"));
...
const fingerprint = properties => (0, _objectHash.default)(properties, {
  algorithm: "sha256",
  ignoreUnknown: true,
...
const validateJWKS = jwks => {
  if (jwks !== undefined) {
    if (!Array.isArray(jwks?.keys) || !jwks.keys.every(_is_plain_object.default)) {
      throw new _errors.InvalidClientMetadata("client JSON Web Key Set is invalid");
    }
  }
};
const nonSecretAuthMethods = new Set(["private_key_jwt", "none", "tls_client_auth", "self_signed_tls_client_auth"]);
const clientEncryptions = ["id_token_encrypted_response_alg", "request_object_encryption_alg", "userinfo_encrypted_response_alg", "introspection_encrypted_response_alg", "authorization_encrypted_response_alg"];
const signAlgAttributes = ["id_token_signed_response_alg", "request_object_signing_alg", "token_endpoint_auth_signing_alg", "userinfo_signed_response_alg", "introspection_signed_response_alg", "authorization_signed_response_alg"];
...
function checkJWK(jwk) {
  try {
    if (!(0, _is_plain_object.default)(jwk)) throw new Error();
    ...
    if (!(jwk.x5c === undefined || Array.isArray(jwk.x5c) && jwk.x5c.every(x => typeof x === "string" && x))) {
      throw new Error();
    }
  } catch {
    throw new _errors.InvalidClientMetadata("client JSON Web Key Set is invalid");
...
function stripFragment(uri) {
  return (0, _nodeUrl.format)(new URL(uri), {
    fragment: false
  });
}
function deriveEncryptionKey(secret, length) {
  ...
  const derived = crypto.createHash(digest).update(secret).digest().slice(0, length);
  return base64url.encodeBuffer(derived);
}
function getClient(provider) {
  const staticCache = new Map();
  const dynamicCache = new _lruCache.LRUCache({
    maxSize: 100
  });
  const Schema = (0, _client_schema.default)(provider);
  const {
    IdToken
  } = provider;
  let adapter;
  function getAdapter() {
    if (!adapter) {
      if ((0, _type_validators.default)((0, _weak_cache.default)(provider).Adapter)) {
        adapter = new ((0, _weak_cache.default)(provider).Adapter)("Client");
      } else {
        adapter = (0, _weak_cache.default)(provider).Adapter("Client");
      }
    }
    return adapter;
  }
  async function sectorValidate(client) {
    if (!(0, _weak_cache.default)(provider).configuration("sectorIdentifierUriValidate")(client)) {
      return;
    }
    const {
      statusCode,
      body
    } = await _request.default.call(provider, {
      method: "GET",
      url: client.sectorIdentifierUri,
      responseType: "json"
    }).catch(err => {
      throw new _errors.InvalidClientMetadata("could not load sector_identifier_uri response", err.message);
    });
    if (statusCode !== 200) {
      throw new _errors.InvalidClientMetadata(`unexpected sector_identifier_uri response status code, expected 200 OK, got ${statusCode} ${_nodeHttp.STATUS_CODES[statusCode]}`);
    }
    try {
      ...
      if (client.responseTypes.length) {
        ...
        if (!match) {
          throw new Error("all registered redirect_uris must be included in the sector_identifier_uri response");
        }
      }
      if (client.grantTypes.includes("urn:openid:params:grant-type:ciba") || client.grantTypes.includes("urn:ietf:params:oauth:grant-type:device_code")) {
        if (!body.includes(client.jwksUri)) {
          throw new Error("client's jwks_uri must be included in the sector_identifier_uri response");
        }
      }
    } catch (err) {
      throw new _errors.InvalidClientMetadata(err.message);
    }
  }
  class ClientKeyStore extends _keystore.default {
    #client;
    #provider = provider;
    ...
    add(key) {
      if (this.client.clientAuthMethod === "self_signed_tls_client_auth" && Array.isArray(key.x5c) && key.x5c.length) {
        ...
        key["x5t#S256"] = (0, _certificate_thumbprint.default)(key.x5c[0]);
      }
      super.add(key);
    }
    async refresh() {
      ...
      if (!this.lock) {
        this.lock = (async () => {
          ...
          const freshUntil = [(0, _epoch_time.default)() + 60];
          if (headers.expires) {
            freshUntil.push((0, _epoch_time.default)(Date.parse(headers.expires)));
          }
          if (headers["cache-control"] && /max-age=(\d+)/.test(headers["cache-control"])) {
            const maxAge = parseInt(RegExp.$1, 10);
            freshUntil.push((0, _epoch_time.default)() + maxAge);
          }
          this.freshUntil = Math.max(...freshUntil.filter(Boolean));
          if (statusCode !== 200) {
            throw new Error(`unexpected jwks_uri response status code, expected 200 OK, got ${statusCode} ${_nodeHttp.STATUS_CODES[statusCode]}`);
          }
          validateJWKS(body);
          this.clear();
          body.keys.map(checkJWK).filter(Boolean).forEach(ClientKeyStore.prototype.add.bind(this));
          delete this.lock;
        })().catch(err => {
          delete this.lock;
          throw new _errors.InvalidClientMetadata("client JSON Web Key Set failed to be refreshed", err.error_description || err.message);
        });
      }
      await this.lock;
    }
    ...
    if (client.clientSecret) {
      if (client.clientAuthMethod === "client_secret_jwt") {
        if (client.clientAuthSigningAlg) {
          ...
          ((0, _weak_cache.default)(provider).configuration("clientAuthSigningAlgValues") || []).forEach(Set.prototype.add.bind(algs));
        }
      }
      ["introspectionSignedResponseAlg", "userinfoSignedResponseAlg", "authorizationSignedResponseAlg", "idTokenSignedResponseAlg", "requestObjectSigningAlg"].forEach(prop => {
        algs.add(client[prop]);
      });
      if (!client.requestObjectSigningAlg) {
        (0, _weak_cache.default)(provider).configuration("requestObjectSigningAlgValues").forEach(Set.prototype.add.bind(algs));
      }
      (0, _weak_cache.default)(provider).configuration("requestObjectEncryptionAlgValues").forEach(Set.prototype.add.bind(algs));
      if ((0, _weak_cache.default)(provider).configuration("requestObjectEncryptionAlgValues").includes("dir")) {
        (0, _weak_cache.default)(provider).configuration("requestObjectEncryptionEncValues").forEach(Set.prototype.add.bind(algs));
      }
      ["idTokenEncryptedResponse", "userinfoEncryptedResponse", "introspectionEncryptedResponse", "authorizationEncryptedResponse"].forEach(prop => {
      ...
      for (const alg of algs) {
        if (!(alg.startsWith("HS") || /^A(\d{3})(?:GCM)?KW$/.test(alg) || /^A(\d{3})(?:GCM|CBC-HS(\d{3}))$/.test(alg))) {
          algs.delete(alg);
        }
      }
      for (const alg of algs) {
        if (alg.startsWith("HS")) {
          ...
          client.symmetricKeyStore.add({
            ...
            use: "enc",
            kty: "oct",
            k: deriveEncryptionKey(client.clientSecret, len)
          });
  ...
  function addStatic(metadata) {
    if (!(0, _is_plain_object.default)(metadata) || !metadata.client_id) {
      throw new _errors.InvalidClientMetadata("client_id is mandatory property for statically configured clients");
    }
    if (staticCache.has(metadata.client_id)) {
      throw new _errors.InvalidClientMetadata("client_id must be unique amongst statically configured clients");
    }
  ...
  (0, _weak_cache.default)(provider).clientAdd = add;
  (0, _weak_cache.default)(provider).clientAddStatic = addStatic;
  (0, _weak_cache.default)(provider).clientRemove = remove;
  class Client {
    #sectorIdentifier = null;
    constructor(metadata, ctx) {
      const schema = new Schema(metadata, ctx);
      Object.assign(this, (0, _map_keys.default)(schema, (value, key) => {
        if (!(0, _weak_cache.default)(provider).RECOGNIZED_METADATA.includes(key)) {
          return key;
        }
        return (0, _camel_case.default)(key);
      }));
      buildAsymmetricKeyStore(this);
      buildSymmetricKeyStore(this);
      validateJWKS(this.jwks);
      if (this.jwks) {
        this.jwks.keys.map(checkJWK).filter(Boolean).forEach(ClientKeyStore.prototype.add.bind(this.asymmetricKeyStore));
      }
    }
    async [backchannel](mode, backchannelAuthenticationRequest, payload) {
      if (!this.backchannelClientNotificationEndpoint || this.backchannelTokenDeliveryMode !== mode || !backchannelAuthenticationRequest || !backchannelAuthenticationRequest.jti || backchannelAuthenticationRequest.kind !== "BackchannelAuthenticationRequest" || !backchannelAuthenticationRequest.params.client_notification_token) {
        throw new TypeError();
      }
      return _request.default.call(provider, {
        method: "POST",
        ...
        if (statusCode !== 204 && statusCode !== 200) {
          const error = new Error(`expected 204 No Content from ${this.backchannelClientNotificationEndpoint}, got: ${statusCode} ${_nodeHttp.STATUS_CODES[statusCode]}`);
    ...
    async backchannelLogout(sub, sid) {
      ...
      logoutToken.set("jti", (0, _nanoid.default)());
      ...
      return _request.default.call(provider, {
        ...
        form: {
          logout_token: await logoutToken.issue({
            use: "logout"
          })
        }
      }).then(response => {
        ...
        if (statusCode !== 200 && statusCode !== 204) {
          const error = new Error(`expected 200 OK from ${this.backchannelLogoutUri}, got: ${statusCode} ${_nodeHttp.STATUS_CODES[statusCode]}`);
          error.response = response;
          throw error;
        }
      });
    ...
    redirectUriAllowed(value) {
      ...
      if (match || this.applicationType !== "native" || parsed.protocol !== "http:" || !_client_attributes.LOOPBACKS.has(parsed.hostname)) {
        return match;
      }
      parsed.port = "";
      return !!this.redirectUris.find(registeredUri => {
    ...
    metadata() {
      return (0, _map_keys.default)(this, (value, key) => {
        const snaked = (0, _snake_case.default)(key);
        if (!(0, _weak_cache.default)(provider).RECOGNIZED_METADATA.includes(snaked)) {
          return key;
        }
        return snaked;
      });
    ...
    get sectorIdentifier() {
      if (this.#sectorIdentifier === null) {
        this.#sectorIdentifier = (0, _sector_identifier.default)(this);
      }
    ...
    compareClientSecret(actual) {
      return (0, _constant_equals.default)(this.clientSecret, actual, 1000);
    }
    checkClientSecretExpiration(message, errorOverride) {
      ...
      if ((0, _epoch_time.default)() - clockTolerance >= this.clientSecretExpiresAt) {
        const err = new _errors.InvalidClient(message, `client_id ${this.clientId} client_secret expired at ${this.clientSecretExpiresAt}`);
        if (errorOverride) {
    ...
    static async find(id) {
      ...
      if (staticCache.has(id)) {
        ...
        if (!(cached instanceof Client)) {
          ...
          Object.defineProperty(client, "noManage", {
            value: true
          });
  ...
  Object.defineProperty(Client, "Schema", {
    value: Schema
  });
  return Client;
}