...
exports.default = checkOpenIdScope;
...
const GATED = ['acr_values', 'claims', 'claims_locales', 'id_token_hint', 'max_age', 'nonce'];
...
function checkOpenIdScope(PARAM_LIST, ctx, next) {
  ...
  if (PARAM_LIST.has('response_type') && ctx.oidc.params.response_type.includes('id_token')) {
    throw new _errors.InvalidRequest('openid scope must be requested for this response_type');
  }
  GATED_CLIENT.forEach(([prop, msg]) => {
    if (ctx.oidc.client[prop]) {
      throw new _errors.InvalidRequest(`openid scope must be requested for clients with ${msg}`);
    }
  });
  GATED.forEach(param => {
    if (ctx.oidc.params[param] !== undefined) {
      throw new _errors.InvalidRequest(`openid scope must be requested when using the ${param} parameter`);
    }
  });
  if (ctx.oidc.route === 'backchannel_authentication') {
    throw new _errors.InvalidRequest('openid scope must be requested for this request');
  }
  return next();
}