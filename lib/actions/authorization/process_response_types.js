...
exports.default = processResponseTypes;
...
async function tokenHandler(ctx) {
  ...
  if (resource) {
    ...
    if (!resourceServer) throw new _errors.InvalidTarget();
...
async function codeHandler(ctx) {
  ...
  const scopeSet = (0, _combined_scope.default)(grant, ctx.oidc.requestParamScopes, ctx.oidc.resourceServers);
...
async function idTokenHandler(ctx) {
  const claims = (0, _filter_claims.default)(ctx.oidc.claims, 'id_token', ctx.oidc.grant);
 ...
 */
async function processResponseTypes(ctx) {
  ...
  if ('id_token' in response) {
    ...
    response.id_token = await response.id_token.issue({
      use: 'idtoken'
    });
  }
  return response;
