...
const init = exports.init = [_no_cache.default, _session.default, parseBody, _assemble_params.default.bind(undefined, new Set(['id_token_hint', 'post_logout_redirect_uri', 'state', 'ui_locales', 'client_id', 'logout_hint'])), _reject_dupes.default.bind(undefined, {}), async function endSessionChecks(ctx, next) {
  const {
    params
  } = ctx.oidc;
  let client;
  ...
  if (ctx.oidc.session.accountId) {
    ...
    await (0, _weak_cache.default)(ctx.oidc.provider).configuration('features.rpInitiatedLogout.logoutSource')(ctx, formHtml);
  } else {
...
const confirm = exports.confirm = [_no_cache.default, _session.default, parseBody, _assemble_params.default.bind(undefined, new Set(['xsrf', 'logout'])), _reject_dupes.default.bind(undefined, {}), async function checkLogoutToken(ctx, next) {
